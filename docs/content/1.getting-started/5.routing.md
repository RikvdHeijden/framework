...
The file-system router is a core feature of Nuxt. Every Vue file created inside the pages/ directory creates a corresponding URL (or route) that displays the content of the file. Nuxt leverages code-splitting on each page by using dynamic imports to ship the minimum amount of JavaScript for the requested route.
...
The `<NuxtLink>` component links pages between routes. It renders an `<a>` tag with a corresponding `href` attribute. Once the application is hydrated, page transitions are performed in JavaScript by updating the browser URL. This prevents full-page refreshes and allows for animated transitions.

When a `<NuxtLink>` enters the viewport on the client side, Nuxt will automatically prefetch components and payloads (generated pages) of the linked pages ahead of time, resulting in faster navigation.
...
The `useRoute()` composable can be used in a `<script setup>` block or the `setup()` method of a Vue component to access the current route details.
